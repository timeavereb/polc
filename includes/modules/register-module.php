...
class Polc_Register_Module
{
    ...
    public function init_register()
    {
        ...
        $this->activation();
        wp_send_json(array("success" => __( 'Register Success Message', 'polc' )));
    }
    ...
    public function activation()
    {
        ...
        $activation_link = add_query_arg(array('key' => $code, 'user' => $this->user), get_permalink(Polc_Settings_Manager::activation_page()));
        add_user_meta($this->user, 'has_to_be_activated', $code, true);

        $headers = array('From: '.$register['sender_name'].' <'. $register['sender_email'].'>');
        $body = $register["body"];
        ...
        wp_mail(
            $this->data['user_email'], $register["subject"], $body, $headers);
    }
}
