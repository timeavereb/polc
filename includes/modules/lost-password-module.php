...
class Polc_Lost_Password_Module
{
    ...
    public static function retrieve()
    {
        ...
        if ($_REQUEST["login"] == ""):
            wp_send_json(["error" => __('Invalid credentials', 'polc')]);
        endif;
        ...
        if (!$user_id):
            ...
            if (!$user_id):
                wp_send_json(["error" => __('The e-mail address or username does not exists.', 'polc')]);
            endif;
        endif;
        ...
        if ($confirm):
            wp_send_json(["error" => __("The account you're trying to retrieve is not yet active.", "polc")]);
        endif;
        ...
        $activation_link = add_query_arg(['key' => $code, 'user' => $user_id], get_permalink(Polc_Settings_Manager::password_reset_page()));
    ...
    public static function reset(){
        ...
        wp_send_json(["success" => __( 'Your password has been changed!','polc')]);
    }
}